...
import { getLeaveBalances } from "@/server/leave-balances";
...
export default async function Requests() {
  ...
  const [types, leave, offtime, correction, balances] = await Promise.all([
  ...
  return <RequestsClient types={types.map((type) => ({ id: type.id, name: type.name, unit: type.unit }))} balances={balances} initial={{ leave, offtime, correction }} />;
}
