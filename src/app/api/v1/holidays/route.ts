...
const schema = z.object({ date: z.coerce.date(), title: z.string().trim().min(2).max(200) });
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ data: await db.holiday.findMany({ where: { companyId: user.companyId, date: { gte: from ? new Date(from) : undefined, lte: to ? new Date(to) : undefined } }, orderBy: { date: "asc" } }) });
}
...
export async function POST(request: Request) {
  ...
  try {
    const holiday = await db.holiday.create({ data: { companyId: user.companyId, date: new Date(Date.UTC(parsed.data.date.getUTCFullYear(), parsed.data.date.getUTCMonth(), parsed.data.date.getUTCDate())), title: parsed.data.title } });
