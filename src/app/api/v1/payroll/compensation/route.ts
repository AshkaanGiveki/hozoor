...
export async function GET() {
  ...
  const rows = await db.compensationProfile.findMany({ where: { companyId: user.companyId }, include: { employee: { select: { id: true, employeeCode: true, firstName: true, lastName: true } }, createdBy: { select: { firstName: true, lastName: true } } }, orderBy: [{ employee: { lastName: "asc" } }, { effectiveFrom: "desc" }] });
...
export async function POST(request: Request) {
  ...
  if (!parsed.success || !isValidDateRange(parsed.data.effectiveFrom, parsed.data.effectiveTo)) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: "Compensation dates or values are invalid." } }, { status: 400 });
