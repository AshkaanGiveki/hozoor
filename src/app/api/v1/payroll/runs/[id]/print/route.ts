...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!run || (!canManagePayroll(user.role) && !(await canAccessEmployee(user, run.employee.id)))) return new NextResponse("Payslip not found", { status: 404 });
  await audit({ companyId: user.companyId, actorId: user.id, action: "payroll.payslip.download", entityType: "PayrollRun", entityId: id, after: { employeeId: run.employee.id, format: "html-print" } });
