...
const schema = z.object({ status: z.enum([PayrollPeriodStatus.IN_REVIEW, PayrollPeriodStatus.APPROVED, PayrollPeriodStatus.PAID, PayrollPeriodStatus.LOCKED, PayrollPeriodStatus.REVERSED, PayrollPeriodStatus.CORRECTED]), reason: z.string().trim().min(3).max(1000).optional() });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (next === PayrollPeriodStatus.REVERSED && !parsed.data.reason) return NextResponse.json({ error: { code: "VALIDATION_ERROR", message: "A reversal reason is required." } }, { status: 400 });
  ...
  const data = { status: next, approvedAt: next === PayrollPeriodStatus.APPROVED ? new Date() : existing.approvedAt, approvedById: next === PayrollPeriodStatus.APPROVED ? user.id : existing.approvedById, paidAt: next === PayrollPeriodStatus.PAID ? new Date() : existing.paidAt, lockedAt: next === PayrollPeriodStatus.LOCKED ? new Date() : existing.lockedAt, reversalReason: next === PayrollPeriodStatus.REVERSED ? parsed.data.reason : existing.reversalReason, reversedAt: next === PayrollPeriodStatus.REVERSED ? new Date() : existing.reversedAt };
  const updated = await db.payrollPeriod.update({ where: { id }, data });
  await audit({ companyId: user.companyId, actorId: user.id, action: next === PayrollPeriodStatus.REVERSED ? "payroll.period.reverse" : "payroll.period.status", entityType: "PayrollPeriod", entityId: id, before: { status: existing.status }, after: { status: next, reason: parsed.data.reason ?? null } });
