...
const transitions: Record<string, string[]> = { CALCULATED: ["IN_REVIEW", "CORRECTED"], IN_REVIEW: ["APPROVED", "CORRECTED"], APPROVED: ["PAID", "REVERSED"], PAID: ["LOCKED", "REVERSED"], LOCKED: ["CORRECTED"], CORRECTED: ["CALCULATED"] };
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!transitions[existing.status]?.includes(next)) return NextResponse.json({ error: { code: "CONFLICT", message: `Cannot move payroll from ${existing.status} to ${next}.` } }, { status: 409 });
