...
const navItems: NavItem[] = [
  ...
  { href: "/schedules", label: "برنامه‌های کاری", caption: "شیفت‌ها و ساعات کار", icon: "settings", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/holidays", label: "تعطیلات", caption: "تقویم سازمان", icon: "settings", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/devices", label: "دستگاه‌ها", caption: "منابع ثبت تردد", icon: "imports", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/audit", label: "گزارش رویدادها", caption: "تاریخچه امنیتی", icon: "reports", roles: ["ADMIN", "HR_ADMIN", "AUDITOR"] },
  { href: "/leave-types", label: "انواع مرخصی", caption: "مدیریت مرخصی", icon: "requests", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/leave-balances", label: "مانده مرخصی", caption: "تخصیص و اصلاح مانده", icon: "reports", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/self-attendance-settings", label: "کنترل موقعیت مکانی", caption: "حضور خوداظهاری", icon: "attendance", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/payroll", label: "حقوق و دستمزد", caption: "محاسبه و پرداخت", icon: "reports", roles: ["ADMIN", "HR_ADMIN"] },
  { href: "/payslips", label: "فیش‌های حقوقی", caption: "حقوق نهایی من", icon: "reports", roles: ["EMPLOYEE", "MANAGER", "ADMIN", "HR_ADMIN"] },
];
...
export default function AppShell({ children, user }: Props) {
  ...
  function toggleTheme() { const next = !darkMode; setDarkMode(next); document.documentElement.classList.toggle("dark", next); localStorage.setItem("ontyme-theme", next ? "dark" : "light"); }
  ...
  return <div className={`${styles.shell} ${styles.shellExpanded}`}>
      ...
      <div className={styles.sidebarBottom}>
        ...
        <div className={styles.accountArea} ref={accountRef}>
          ...
          {accountOpen && <div className={styles.accountMenu} role="menu">
            ...
            <div className={styles.accountMenuFooter}><span>OnTyme</span><span>نسخهٔ سازمانی</span></div>
          </div>}
        </div>
      </div>
    </aside>
    ...
    <div className={styles.content}>
      ...
      <footer className={styles.footer}>
        ...
        <span className={styles.copyright}>© {new Date().getFullYear()} OnTyme</span>
