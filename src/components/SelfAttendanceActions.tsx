...
export default function SelfAttendanceActions() {
  ...
  return <div style={{ display: "flex", gap: 8, alignItems: "center" }}><button className="button secondary" type="button" disabled={busy} onClick={() => void record("IN")}>ثبت ورود</button><button className="button primary" type="button" disabled={busy} onClick={() => void record("OUT")}>ثبت خروج</button>{message && <small role="status">{message}</small>}</div>;
}
