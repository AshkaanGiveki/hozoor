...
export async function calculatePayrollPeriod(periodId: string, companyId: string) {
  ...
  for (const employee of employees) {
    ...
    const gross = rounded(base.plus(componentEarnings).plus(overtime).plus(adjustmentEarnings));
    ...
    const taxable = rounded(base.plus(earnings.filter((item) => item.taxable !== false).reduce((sum, item) => sum.plus(money(item.amount)), money(0))).plus(overtime).plus(taxableAdjustments));
    const calculatedInsurable = rounded(base.plus(earnings.filter((item) => item.insurable !== false).reduce((sum, item) => sum.plus(money(item.amount)), money(0))).plus(overtime).plus(insurableAdjustments));
    ...
    const net = rounded(gross.minus(employeeInsurance).minus(tax).minus(componentDeductions).minus(adjustmentDeductions));
    const lines = [
      ...
      ...components.map((item) => ({ type: (item.type ?? "ALLOWANCE") as "ALLOWANCE" | "BONUS" | "COMMISSION" | "BENEFIT" | "OTHER_EARNING" | "LOAN_REPAYMENT" | "ADVANCE_REPAYMENT" | "OTHER_DEDUCTION", code: item.code, label: item.label, amount: rounded(isEarningComponent(item) ? money(item.amount) : money(item.amount).neg()), sourceData: { compensationProfileId: compensation.id, componentType: item.type ?? "ALLOWANCE", taxable: item.taxable !== false, insurable: item.insurable !== false } })),
