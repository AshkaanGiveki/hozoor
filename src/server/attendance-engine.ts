...
export function calculateAttendance(input: AttendanceInput): AttendanceResult {
  ...
  if (!rule.isWorkingDay && !input.holiday) return emptyResult(DayStatus.WEEKLY_OFF);
  ...
  if (!sessions.length) return { ...emptyResult(input.holiday ? DayStatus.HOLIDAY : DayStatus.ABSENT), requiredMinutes: rule.requiredMinutes, minimumMinutes: rule.minimumMinutes ?? rule.requiredMinutes, anomalies: input.holiday ? [] : ["MISSING_IN","MISSING_OUT"] };
  ...
  const completedSessions = completed.map((session) => ({ start: session.startAt, end: session.endAt! }));
  const nightWorkMinutes = completedSessions.reduce((sum, session) => sum + overlapMinutes(session.start, session.end, input.date, 0, 6 * 60) + overlapMinutes(session.start, session.end, input.date, 22 * 60, 24 * 60), 0);
  const fridayWorkMinutes = input.date.getUTCDay() === 5 ? validWorkMinutes : 0;
  const holidayWorkMinutes = input.holiday ? validWorkMinutes : 0;
  ...
  const status = input.holiday ? DayStatus.HOLIDAY : input.leaveMinutes && validWorkMinutes < minimumMinutes ? DayStatus.PARTIAL_LEAVE : input.leaveMinutes && validWorkMinutes >= minimumMinutes ? DayStatus.ON_LEAVE : input.offTimeMinutes ? DayStatus.PARTIAL_OFF_TIME : anomalies.length ? DayStatus.INCOMPLETE : validWorkMinutes < minimumMinutes ? DayStatus.INSUFFICIENT_TIME : DayStatus.PRESENT;
  return { status, requiredMinutes: rule.requiredMinutes, minimumMinutes, firstIn, lastOut, rawPresenceMinutes, validWorkMinutes, breakMinutes, lateMinutes, earlyDepartureMinutes, deficitMinutes, rawOvertimeMinutes, calculatedOvertimeMinutes, nightWorkMinutes, fridayWorkMinutes, holidayWorkMinutes, anomalies, sessions };
}
function overlapMinutes(start: Date, end: Date, day: Date, fromMinutes: number, toMinutes: number) { const windowStart = new Date(day); windowStart.setUTCHours(Math.floor(fromMinutes / 60), fromMinutes % 60, 0, 0); const windowEnd = new Date(day); windowEnd.setUTCHours(Math.floor(toMinutes / 60), toMinutes % 60, 0, 0); return Math.max(0, Math.round((Math.min(end.getTime(), windowEnd.getTime()) - Math.max(start.getTime(), windowStart.getTime())) / 60000)); }
function emptyResult(status: DayStatus): AttendanceResult { return { status, requiredMinutes: 0, minimumMinutes: 0, firstIn: null, lastOut: null, rawPresenceMinutes: 0, validWorkMinutes: 0, breakMinutes: 0, lateMinutes: 0, earlyDepartureMinutes: 0, deficitMinutes: 0, rawOvertimeMinutes: 0, calculatedOvertimeMinutes: 0, nightWorkMinutes: 0, fridayWorkMinutes: 0, holidayWorkMinutes: 0, anomalies: [], sessions: [] }; }
export function toAttendanceDayData(result: AttendanceResult): Prisma.AttendanceDayUncheckedCreateInput { return { status: result.status, requiredMinutes: result.requiredMinutes, minimumMinutes: result.minimumMinutes, firstIn: result.firstIn, lastOut: result.lastOut, rawPresenceMinutes: result.rawPresenceMinutes, validWorkMinutes: result.validWorkMinutes, breakMinutes: result.breakMinutes, lateMinutes: result.lateMinutes, earlyDepartureMinutes: result.earlyDepartureMinutes, deficitMinutes: result.deficitMinutes, rawOvertimeMinutes: result.rawOvertimeMinutes, calculatedOvertimeMinutes: result.calculatedOvertimeMinutes, nightWorkMinutes: result.nightWorkMinutes, fridayWorkMinutes: result.fridayWorkMinutes, holidayWorkMinutes: result.holidayWorkMinutes, approvedOvertimeMinutes: 0, leaveMinutes: 0, offTimeMinutes: 0, anomalies: result.anomalies } as Prisma.AttendanceDayUncheckedCreateInput; }
